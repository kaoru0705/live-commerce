...
/**
 * 사용법
 * - 기본: <MediaStage mode="player" />  (LivePlayer)
 * - 웹캠으로 전환: <MediaStage mode="webcam" />
 *
 * 혹은 페이지에서 const MEDIA_MODE = "player"만 바꾸면 됨.
 */
const MEDIA_MODE = "player"; // "player" | "webcam"
      ...
      <div className="container py-5">
        <div className="row g-4">
          ...
          <aside className="col-lg-5 col-xl-3 wow fadeInUp" data-wow-delay="0.1s">
            ...
            <ColorFilter title="Select By Color" options={colors} value={selectedColor} onChange={setSelectedColor} />
          ...
          <section className="col-lg-7 col-xl-9 wow fadeInUp" data-wow-delay="0.1s">
            <div className="row g-4 single-product">
              {/* LEFT: 미디어 영역 (기본 LivePlayer, 필요시 WebCam으로 교체 가능) */}
              <div className="col-xl-6">
                <MediaStage
                  ...
                  player={{
                    title: "LIVE",
                    // 1) YouTube embed URL 예시
                    // src: "https://www.youtube.com/embed/jfKfPfyJRdk?autoplay=1&mute=1&playsinline=1",
                    // 2) HLS(.m3u8)나 mp4 URL이면 video 태그로 재생되게 설정할 수도 있음
                    src: "https://www.youtube.com/embed/jfKfPfyJRdk?autoplay=1&mute=1&playsinline=1",
                  }}
                  // WebCam 설정: 필요하면 audio 끄기 가능
                  webcam={{
                    withAudio: true,
                    preferFront: true,
                  }}
...
function MediaStage({ mode = "player", player, webcam }) {
  // 여기서만 컴포넌트 스위칭
  if (mode === "webcam") return <WebcamLivePlayer {...webcam} />;
  return <LivePlayer {...player} />;
}

/**
 * LivePlayer
 * - 기본은 YouTube embed(iframe)로 재생
 * - 만약 src가 .mp4 / .m3u8(브라우저 지원/ hls.js 필요)라면 video 플레이어로 바꿀 수도 있음
 */
function LivePlayer({ title = "LIVE", src }) {
  const isLikelyVideoFile = typeof src === "string" && (src.endsWith(".mp4") || src.endsWith(".webm") || src.endsWith(".ogg"));

  return (
    <div className="bg-light rounded p-2">
      <div className="position-relative rounded overflow-hidden" style={{ width: "100%", paddingTop: "56.25%" }}>
        {isLikelyVideoFile ? (
          <video
      ...
      <div className="d-flex justify-content-between align-items-center mt-2 px-1">
        <div className="d-flex align-items-center gap-2">
          <small className="fw-bold text-danger">● LIVE</small>
          <small className="text-muted">Live player running</small>
        </div>

        <span className="badge bg-secondary">LivePlayer</span>
      </div>
    </div>
  );
}
...
/**
 * WebCam(내장 카메라) 라이브 미리보기
 * - HTTPS 또는 localhost에서만 동작
 */
function WebcamLivePlayer({ withAudio = true, preferFront = true }) {
  ...
  const start = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError("이 브라우저는 카메라 접근(getUserMedia)을 지원하지 않습니다.");
      return;
    }
    ...
    try {
      ...
      setError("카메라를 시작할 수 없습니다. (권한 허용/HTTPS 또는 localhost 환경/카메라 점유 여부 확인)");
      setIsOn(false);
    ...
    <div className="bg-light rounded p-2">
      <div className="position-relative rounded overflow-hidden" style={{ width: "100%", paddingTop: "56.25%" }}>
        <video
          ...
          className="position-absolute top-0 start-0 w-100 h-100"
          playsInline
          muted // 내 스피커로 마이크 소리 재생 방지(하울링). 송출 자체와는 별개
          controls={false}
          autoPlay
        />
      </div>
      ...
      <div className="d-flex justify-content-between align-items-center mt-2 px-1">
        <div className="d-flex align-items-center gap-2">
          <small className={`fw-bold ${isOn ? "text-danger" : "text-muted"}`}>● LIVE</small>
          <small className="text-muted">{isOn ? "Camera streaming..." : "Camera is off"}</small>
        </div>
        ...
        <div className="d-flex gap-2 align-items-center">
          ...
          <span className="badge bg-secondary">WebCam</span>
        </div>
      </div>
...
function ProductSummary({ qty, onMinus, onPlus }) {
  return (
    <>
      ...
      <Link to="/cart" className="btn btn-primary border border-secondary rounded-pill px-4 py-2 mb-4 text-primary">
...
function DescriptionBlock() {
  return (
    <>
      <p>
        Our new <b className="fw-bold">HPB12 / A12 battery</b> is rated at 2000mAh and designed to power up Black and Decker / FireStorm
        line of 12V tools allowing users to run multiple devices off the same battery pack. The HPB12 is compatible with the following
        Black and Decker power tool models:
      </p>
...
function ReviewItem({ r }) {
  return (
    <div className="d-flex mb-4">
      <img src={r.avatar} className="img-fluid rounded-circle p-3" style={{ width: 100, height: 100 }} alt={r.name} />
      <div>
  ...
  return (
    <>
      {Array.from({ length: outOf }).map((_, i) => (
        <i key={i} className={`fa fa-star ${i < value ? activeClass : ""}`} aria-hidden="true"></i>
      ))}
    </>
  );
}
